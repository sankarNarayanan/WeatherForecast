...
@objc protocol OperationQueueDelegate: NSObjectProtocol {
    @objc optional func operationQueue(operationQueue: AppOperationQueue, willAddOperation operation: Operation)
    @objc optional func operationQueue(operationQueue: AppOperationQueue, operationDidFinish operation: Operation, withErrors errors: [NSError])
}
...
*/
class AppOperationQueue: OperationQueue {
    weak var delegate: OperationQueueDelegate?
    
    override func addOperation(_ operation: Operation) {
        if let appOp = operation as? AppOperation {
            ...
            appOp.addObserver(observer: delegate)
            
            // Extract any dependencies needed by this operation.
            let dependencies = appOp.conditions.flatMap {
                $0.dependencyForOperation(operation: appOp)
            }
                
            for dependency in dependencies {
                appOp.addDependency(dependency)
            ...
            */
            let concurrencyCategories: [String] = appOp.conditions.flatMap { condition in
            ...
            if !concurrencyCategories.isEmpty {
                ...
                exclusivityController.addOperation(operation: operation, categories: concurrencyCategories)
                
                appOp.addObserver(observer: BlockObserver { operation, _ in
