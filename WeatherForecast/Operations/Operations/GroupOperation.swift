 ...
 */
var internalQueue = AppOperationQueue()

class GroupOperation: AppOperation {
    

    
    let startingOperation = BlockOperation(block: {})
...
extension GroupOperation: OperationQueueDelegate {
    final func operationQueue(operationQueue: AppOperationQueue, willAddOperation operation: Operation) {
        assert(!self.finishingOperation.isFinished && !self.finishingOperation.isExecuting, "cannot add new operations to a group after the group has completed")
    ...
    final func operationQueue(operationQueue: AppOperationQueue, operationDidFinish operation: Operation, withErrors errors: [NSError]) {
