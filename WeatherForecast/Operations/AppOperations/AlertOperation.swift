...
@available(iOS 8.0, *)
class AlertOperation: AppOperation {
    // MARK: Properties
    ...
    var aLabel: String? {
        get {
            return alertController.accessibilityLabel ?? ""
        }
        
        set {
            alertController.accessibilityLabel = aLabel ?? ""
            alertController.accessibilityValue = title ?? ""
            name = newValue
        }
    }
    ...
    func addAction(title: String, style: UIAlertActionStyle = .default, withLabel: String, handler: @escaping (AlertOperation) -> Void = { _ in }) {
        let action = UIAlertAction(title: title, style: style) { [weak self] _ in
            if let strongSelf = self {
                handler(strongSelf)
            }
        ...
        action.accessibilityLabel = withLabel
        action.accessibilityValue = title
        alertController.addAction(action)
    }
